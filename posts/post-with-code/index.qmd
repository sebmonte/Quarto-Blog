...
Finally, let's do some diagnostics to evaluate whether some of the other assumptions relevant to mixed models hold with our data. check_model from the performance package is a nice way to run many of these at once.
...
```{r}
library(glmer) #generalized linear model. We are loading this in now because for the first step, our outcome isn't going to be continuous. We are going to ask 'does a participant say something is important at all'
df_long <- df_long |>
...
model_hurdle1 <- glmer(
  ...
  family = binomial, 
  data = df_long
) #again, this model is for the binary outcome of whether a feature is said to matter
...
In this tutorial, we walked through using mixed models with random intercepts to fit data from a real experiment. We found a significant...
